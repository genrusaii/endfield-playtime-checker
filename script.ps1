...
& {
    Write-Host "Scanning for Arknights: Endfield logs..." -ForegroundColor Cyan
    ...
    # Стандартный путь AppData
    $searchPaths.Add("C:\Users\$user\AppData\LocalLow\Gryphline")

    # Автопоиск по всем дискам на случай кастомной установки (например, на диск D:)
    ...
    if (-not $files) { 
        Write-Host "Gryphline log files not found. Use -CustomPath 'C:\YourPath' to specify manually." -ForegroundColor Red; 
        return 
    }
    ...
    # Вывод аналитики
    Write-Host "`n=-- ARKNIGHTS: ENDFIELD ANALYTICS (v1.2) --=" -ForegroundColor Cyan
    ...
    Write-Host " > Longest Session      : " -NoNewline; Write-Host "$([math]::Floor($sortedSess[-1].Duration.TotalHours))h $($sortedSess[-1].Duration.Minutes)m" -ForegroundColor Green -NoNewline; Write-Host "  (Date: $($sortedSess[-1].Start.ToString('yyyy-MM-dd')))"
    ...
    Write-Host " > Night Gaming (22-06) : $nightPct% of time"
    ...
    Write-Host " > Standard (30m-3h)    : $($sessions | Where-Object { $_.Mins -ge 30 -and $_.Mins -le 180 } | Measure-Object | Select-Object -ExpandProperty Count)"
    Write-Host " > Hardcore Marathons   : $($sessions | Where-Object Mins -gt 180 | Measure-Object | Select-Object -ExpandProperty Count)"
    ...
    foreach ($h in $hourly) {
        ...
        Write-Host " $hourStr │ " -NoNewline -ForegroundColor Cyan
        Write-Host "$($bar.PadRight(20))" -NoNewline -ForegroundColor DarkCyan
        Write-Host " [$($h.Count)]"
    }
    ...
    'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday' | ForEach-Object {
        ...
        if ($dMins) { 
            ...
            Write-Host " > $($day.PadRight(10)) : " -NoNewline
    ...
    $sessions | Group-Object { $_.Start.ToString('yyyy-MM (MMMM)') } | Sort-Object Name | ForEach-Object {
        ...
        Write-Host " > $($_.Name.PadRight(18)) : " -NoNewline
        Write-Host "$([math]::Floor($span.TotalHours))h $($span.Minutes)m" -ForegroundColor Green
    }
